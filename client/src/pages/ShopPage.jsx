...
const ShopPage = () => {
  ...
  const categoryDescription = useSelector(
    (state) => state.shop.categoryDescription
  );
  const isProductsLoading = useSelector(
    (state) => state.shop.isProductsLoading
  );
  const isCategoryLoading = useSelector(
    (state) => state.shop.isCategoryLoading
  );
  ...
  useEffect(() => {
    if (categoryId === "all") {
      dispatch(
        loadProducts({ page: 1, limit, minPrice, maxPrice, ratings, sortBy })
      ).then((data) => {
  ...
  const fetchByPage = (page) => {
    if (categoryId === "all") {
      dispatch(
        loadProducts({ page: page, limit, minPrice, maxPrice, ratings, sortBy })
      );
    } else {
      dispatch(
        loadProductsByCategory({
