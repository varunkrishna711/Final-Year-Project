...
const OrderService = require("../services/order-service");
const { validateEmail } = require("../helpers/validateEmail");
const { Order } = require("../db/models/models"); // Assuming you have a Mongoose Order model
...
class OrderController {
  async create(req, res, next) {
    ...
    if (!firstName || !lastName) {
      return next(
        ApiError.badRequest("Firstname and lastname cannot be empty")
      );
    }
    if (!country) {
      return next(ApiError.badRequest("Country cannot be empty"));
  ...
  async getOrderStatistic(req, res, next) {
    ...
    try {
      const orderStatistic = await OrderService.getOrderStatistic(
        startDate,
        lastDate
      );
      return res.json(orderStatistic);
  ...
  async getSaleStatistic(req, res, next) {
    ...
    try {
      const saleStatistic = await OrderService.getSaleStatistic(
        startDate,
        lastDate
      );
      return res.json(saleStatistic);
